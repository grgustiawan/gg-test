            ...
            <div class="p-5">
                <div class="preview relative [&amp;.hide]:overflow-hidden [&amp;.hide]:h-0">
                    <div class="mt-1">
                        ...
                        <select readonly data-tw-merge="" v-model="user.id_staff" aria-label="Default select example" class="disabled:bg-slate-100 disabled:cursor-not-allowed disabled:dark:bg-darkmode-800/50 [&amp;[readonly]]:bg-slate-100 [&amp;[readonly]]:cursor-not-allowed [&amp;[readonly]]:dark:bg-darkmode-800/50 transition duration-200 ease-in-out w-full text-sm border-slate-200 shadow-sm rounded-md py-2 px-3 pr-8 focus:ring-4 focus:ring-primary focus:ring-opacity-20 focus:border-primary focus:border-opacity-40 dark:bg-darkmode-800 dark:border-transparent dark:focus:ring-slate-700 dark:focus:ring-opacity-50 group-[.form-inline]:flex-1 mt-2 sm:mr-2">
...
export default {
    ...
    mounted(){
        this.user.id_staff = this.userdata.id_staff
        this.user.hak_akses = this.userdata.hak_akses
        this.user.status_akun = this.userdata.status_akun
        this.getUsers()
    },
    methods: {
        closeModal(){
        ...
        async submitUser(){
            try {
                this.isLoading = true;
                const {data} = await axios.patch('/user/' + this.userdata.id , this.user, {
                    headers: {
                        'Accept': 'application/json',
                        'Authorization': 'Bearer ' + this.$store.getters.GET_AUTH_TOKEN
                    },
